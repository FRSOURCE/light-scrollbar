...
export const attach = (containerElement: HTMLElement, config: config = {
  ...
  wrapper.style.setProperty('--light-scrollbar-y-width', `${config.scrollBarYWidth}px`)
  wrapper.style.setProperty('--light-scrollbar-x-height', `${config.scrollBarXHeight}px`)
  ...
  const hoverScrollbar = () => {
    (['x', 'y'] as ('x' | 'y')[]).forEach((dir) => {
      ...
      const scrollbarWidthOrHeight = dir === 'y' ? 'scrollBarYWidth' : 'scrollBarXHeight'
      const widthOrHeight = dir === 'y' ? 'width' : 'height'

      data.scrollbar[dir].isHovered =
        data.mouse[reverse] <= data.container[widthOrHeight]
        && data.mouse[reverse] >= data.container[widthOrHeight] - config[scrollbarWidthOrHeight]
  ...
  const calculateHeightOfScrollbars = () => {
    ...
    (['x', 'y'] as ('x' | 'y')[]).forEach((dir) => {
      ...
      data.scrollbar[dir].long.realPercent = data.container[long] / data.content[long] * 100;
      const realPerc = data.scrollbar[dir].long.realPercent;
      data.scrollbar[dir].long.percent =
        realPerc >= data.minLongPercent
          ? realPerc
          : data.minLongPercent;
      data.scrollbar[dir].long.pixel = data.scrollbar[dir].long.percent / 100 * data.container[long];

      const perc = data.scrollbar[dir].long.percent;
      wrapper.style.setProperty(`--light-scrollbar-${dir}-${long}`,
      `${perc === 100 ? 0 : perc}%`)
    })
  }
  ...
  const calculateTopOfScrollbar = (e?: Event) => {
    (['x', 'y'] as ('x' | 'y')[]).forEach((dir) => {
      ...
      const scrollTopLeft = (e?.target as HTMLElement)?.[leftOrTop] || containerElement[leftOrTop];
      ...
      data.scrollbar[dir].gap.toContent.percent = scrollTopLeft / data.content[long];
      data.scrollbar[dir].gap.toContainer.pixel  =
        (((scrollTopLeft + data.container[long] - diffPixels) / data.content[long])
         * (data.container[long] - diffPixels)) - (data.scrollbar[dir].long.pixel - diffPixels)

      const isTop = dir === 'y' ? 'top' : 'left'
      wrapper.style.setProperty(`--light-scrollbar-${dir}-${isTop}`, `${data.scrollbar[dir].gap.toContainer.pixel}px`)
    });
   }
  ...
  const scrollOnDrag = (event: MouseEvent, prevent = false) => {
    (['x', 'y'] as ('x' | 'y')[]).forEach((dir) => {
      ...
      if(data.rail[dir].isStartingPointHover) {
        ...
        const perc = (data.mouse[dir] + data.scrollbar[dir].dragDiff) / data.container[percWidthOrHeight];
        data.scrollbar[dir].gap.toContent.pixel = data.content[percWidthOrHeight] * perc
        const leftOrTop = dir === 'y' ? 'scrollTop' : 'scrollLeft';
  ...
  wrapper.addEventListener('mousemove', containerMouseMoveHandler, {passive: true})
